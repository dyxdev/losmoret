...
export async function userSignup(data: UserSignup): Promise<CommonResult | GeneralApiProblem> {
  const response = await api.apiPostWrapper<CommonResult>("/auth/signup", {
    params: data,
  })
  return response
}
...
export async function login(data: UserSignin): Promise<LoginResponse | GeneralApiProblem> {
  const response = await api.apiPostWrapper<LoginResponse>("auth/signin", {
    jsonrpc: "2.0",
    params: data,
  })
...
export async function logout(): Promise<CommonResult | GeneralApiProblem> {
  const response = await api.apiPostWrapper<CommonResult>("/auth/logout", undefined)
  return response
}

export async function updateProfile(data: UserProfile): Promise<CommonResult | GeneralApiProblem> {
  const response = await api.apiPostWrapper<CommonResult>("/user/profile", { params: data })
  return response
}
...
export async function getAddress(): Promise<ResultClass<Address[]> | GeneralApiProblem> {
  const response = await api.apiGetWrapper<ResultClass<Address[]>>("/user/address", {})
  return response
}
...
export async function postAddress(
  ...
  const response = await api.apiPostWrapper<ResultClass<Address>>("/user/address", {
    jsonrpc: "2.0",
    params: data,
  })
  return response
...
export async function getStateByCountry(id: number): Promise<
  | GeneralApiProblem
  | ResultClass<
      {
        id: number | string
    ...
    >
> {
  const response = await api.apiGetWrapper<ResultClass<{ id: number | string; name: string }[]>>(
    `/address/country/${id}/state`,
    {},
  )
  return response
}
